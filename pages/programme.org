...
une application de profilage, utiliser ce modèle de faute sur l'application sensible (en simulation
par exemple) afin de déterminer les chemins d'attaques possibles et
exploiter l'effet de la faute injectée sur le composant cible. Dans la littérature,
ce travail reste principalement porté sur la sécurité des implémentations cryptographiques
implémentées sur des micro-contrôleurs.
...
Raspberry Pi OS Lite  pour Raspberry Pi 4.
Nous montrons comment il est possible de transposer une caractérisation
...
Notre méthode est suffisamment efficace pour
